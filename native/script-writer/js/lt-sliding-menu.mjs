...
export default class LTSlidingMenu extends HTMLElement {
    #$selected;
    #$list;
    static get observedAttributes() { return ["side", "show", "gap"] }
    ...
    constructor() {
        ...
        // Try to get the side, show, gap attributes and set the class on the list
        ...
        const gap = this.hasAttribute("gap") ? this.getAttribute("gap") : "0";
        ...
        switch (name) {
            ...
            case "gap":
                for (let li of this.#$list.children) {
                    li.style = `margin-${SIDE_INVERTER.get(this.side)}: ${newValue};`;
                }
                break;
            default:
        }
